    ...
    describe('tokenByIndex', function () {
      ...
      [firstTokenId, secondTokenId].forEach(function (tokenId) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
          const createTokenId = new BN(300);
          const anothercreateTokenId = new BN(400);

          await this.token.burn(tokenId);
          await this.token.mint(newOwner, createTokenId);
          await this.token.mint(newOwner, anothercreateTokenId);
          ...
          const expectedTokens = [firstTokenId, secondTokenId, createTokenId, anothercreateTokenId].filter(
            x => (x !== tokenId),
          );
