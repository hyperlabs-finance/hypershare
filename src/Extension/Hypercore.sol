...
/**
	...
	TODO, reduce this to a generalised interface that can be inhereted by all HypershareExtensions.

 */

contract KaliDAOvesting {
        ...
        virtual
    {
        ...
        if ( accounts.length != amounts.length ||  amounts.length != startTimes.length ||  startTimes.length != endTimes.length) 
