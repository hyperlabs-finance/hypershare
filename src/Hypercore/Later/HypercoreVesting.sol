...
contract HyperDAOVesting is Hypercore {

    ////////////////
    // EVENTS
    ////////////////

    event HypercoreSet(address indexed dao, address[] accounts, uint256[] amounts, uint256[] startTimes, uint256[] endTimes);
    event HypercoreCalled(address indexed dao, uint256 vestingId, address indexed member, uint256 indexed amountOut);
    ...
    function setHypercore(
        ...
        virtual
    {
        (address[] memory accounts, uint256[] memory amounts, uint256[] memory startTimes, uint256[] memory endTimes) = abi.decode(hypercoreData, (address[], uint256[], uint256[], uint256[]));
        ...
        emit HypercoreSet(msg.sender, accounts, amounts, startTimes, endTimes);
    }
    ...
    function callHypercore(
        ...
        uint256 vestingId = abi.decode(hypercoreData, (uint256));
        ...
        emit HypercoreCalled(msg.sender, vestingId, account, amount);
    }
}
