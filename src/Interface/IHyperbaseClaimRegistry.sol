...
interface IHyperbaseClaimRegistry {
    ...
    function checkClaimValidById(address subject, bytes32 claimId) external view returns (bool claimValid);
    function checkClaimValid(address subject, uint256 topic, bytes memory sig, bytes memory data) external view returns (bool claimValid);
    function checkClaimRevoked(bytes memory sig) external view returns (bool);
